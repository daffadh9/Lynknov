...
export default function RegisterPage() {
  ...
  const registerTitle = (
    <>
      Ubah interaksi pertama jadi <span className="relative inline-block text-transparent bg-clip-text bg-gradient-to-r from-white via-white/80 to-white/40 overflow-hidden">
        peluang bisnis.
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent skew-x-12 translate-x-[-150%] animate-[shimmer_3s_infinite_ease-in-out]"></div>
      </span>
    </>
  );
  ...
  return (
    <AuthLayout
      title={registerTitle}
      description="Bangun sistem kehadiran digital yang bukan sekadar tautan profil, tapi mesin konversi yang bekerja secara profesional untuk Anda."
      features={["Setup dalam hitungan menit", "Alur kerja terpandu", "Fokus pada konversi"]}
      metricsType="growth"
      animationType="up"
    >
      <div className="flex flex-col space-y-8">
        <div className="flex flex-col space-y-2">
          <h2 className="text-2xl font-semibold tracking-tight text-white">Satu akun, banyak peluang</h2>
          <p className="text-sm text-[var(--color-text-secondary)]">
            Mulai bangun kehadiran digital Anda hari ini.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-5">
          <div className="space-y-4">
            ...
            <div className="space-y-1.5 relative group">
              <label htmlFor="name" className={`text-sm font-medium transition-colors duration-300 ${focusedField === 'name' ? 'text-[#a78bfa]' : 'text-[var(--color-text-secondary)]'}`}>Nama Lengkap</label>
        ...
        <div className="relative flex items-center py-2">
          ...
          <span className="flex-shrink-0 mx-4 text-xs text-[var(--color-text-tertiary)] uppercase tracking-wider">Atau daftar dengan</span>
          <div className="flex-grow border-t border-white/5"></div>
        </div>
        ...
        <p className="text-center text-sm text-[var(--color-text-secondary)]">
          Sudah punya akun?{" "}
          <Link href="/login" className="font-medium text-white hover:text-[#a78bfa] transition-colors">
            Masuk
          </Link>
        </p>
      </div>
